...
Note: WASM sandboxing, dynamic loading, security not in Session 2.
...
· AST caching for repeated formulas (Phase 12)
· Short-circuit evaluation for boolean ops
· Vectorized operations for large arrays
· Benchmark suite with criterion (Phase 14)
...
· Unit tests for AST parsing of every new node
· Integration tests for higher-order functions with lambda
· Roundtrip serialization tests
· Fuzz testing for access chain parser
· CI: cargo fmt, cargo clippy, cargo test on pure Rust toolchain (no C dependency)

---

13) Migration from V1

· All original APIs remain valid
· Value::DateTime and Value::Duration added, but not mandatory
· Original date functions returning strings remain (but use native internally for speed)
· Plugin SDK is strictly opt-in
