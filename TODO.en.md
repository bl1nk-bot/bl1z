...
## 🟢 Phase 10.5: Missing Math + String Builtins 🆕 (2 weeks) ✅ DONE
...
- [x] `sqrt(n)`, `pow(base, exp)`, `abs(n)` ✅ (Exists)
- [x] `sin(n)`, `cos(n)`, `tan(n)` (Using `libm` or pure Rust implementation)
...
## 🟢 Phase 11: Advanced Data Types (2 weeks) ✅ DONE

Duration: 2 weeks
> Note: Refactor date builtins from string wrapping → native DateTime/Duration

- [x] **11.1** Add `Value::DateTime(jiff::Timestamp)` and `Value::Duration(jiff::SignedDuration)`
- [x] **11.2** Add `Value::Set(HashSet<Value>)` and `Value::Range { start, end, step }`
- [x] **11.3** Refactor date builtins: `now()` → return `Value::DateTime`, `date()` → parse → `Value::DateTime`
- [x] **11.4** Refactor `date_add()`, `date_diff()` → operate on native types
- [x] **11.5** Add @ operator: `@2024-01-01` → DateTime literal
...
## 🟢 Phase 12: Serialization & Caching (1.5 weeks) ✅ DONE
...
## 🔵 Phase 14: Performance & Optimization (2 weeks) 🔄 IN PROGRESS
