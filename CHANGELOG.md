...
- **JSON Plugins**: Manifest-driven function registration via `load_json_plugin` / `JsonPlugin`, with engine-version pinning. Script functions delegate to external interpreters (python3, python, node, deno, bun) via JSON on stdin/stdout.
